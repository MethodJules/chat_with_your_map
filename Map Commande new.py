...
class LayerResolver:
    ...
    def resolve(self, user_input: str) -> List[str]:
        ...
        # Zuerst nach exakten Keywords suchen
        ...
        # Wenn keine Kategorie erkannt → versuche mit LLM-ähnlicher Logik
        if not found_category:
            inferred_category = self.infer_category_with_llm(user_input)
            if inferred_category in self.keyword_mapping:
                filtered = self.layer_df[self.layer_df['Kategorie'].str.lower().str.contains(inferred_category)]
                matches.extend(filtered['Layer ID'].tolist())

        return list(set(matches))

    def infer_category_with_llm(self, user_input: str) -> str:
        """
        Vereinfachte LLM-Simulation zur Synonymerkennung.
        """
        synonyms = {
            "öffis": "öpnv",
            "polente": "polizei",
        ...
        return ""  # Keine bekannte Zuordnung
...
def main():
    ...
    commands = [
        ...
        "Zeig mir die nebenste Öffis.",  # ← Umgangssprachlich!
        "Ich will die Spieli sehen.",     # ← Umgangssprachlich für Spielplatz
        "Wo ist die Polente?"             # ← Umgangssprache für Polizei
    ]
